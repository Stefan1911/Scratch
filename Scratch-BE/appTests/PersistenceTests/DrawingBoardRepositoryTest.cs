...
namespace appTests.PersistenceTests
{
    public class DrawingBoardRepositoryTest
    {
        ...
        [Fact]
        public async void GetCollectionTest()
        {
            var boardRepository = appTestDependencyHelper.drawingBoardRepository;
            var projectRepository = appTestDependencyHelper.projectRepository;
            ...
            A.Configure<DrawingBoardModel>()
                 .Fill(c => c.Id, () => { return null; });
            var boards = A.ListOf<DrawingBoardModel>(5);

            var boardsBefore= await boardRepository.GetCollecionAsync(project.Id);

            foreach (var board in boards)
            {
                await boardRepository.AddAsync(board, project.Id);
            }

            var boardCollection =await  boardRepository.GetCollecionAsync(project.Id);

            Assert.Equal(boardCollection.Count(), boardsBefore.Count() + boards.Count());
